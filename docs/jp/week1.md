# Week 1: 組立と立ち上げ {#page:w1}

## Week 1 の概要と到達目標 {#w1-overview}
...
## 電子部品の解説 {#w1-electronic-components}
...
### 抵抗とコンデンサの表記（値と極性） {#sec:passive_markings}

#### 抵抗：抵抗値の見分け方 {#w1-resistors}
...
#### コンデンサ：容量表示と極性の見方 {#w1-capacitors}
...
### モータドライバ（DRV8832） {#w1-motor-driver}
...
### フォトインタラプタと傾斜センサ {#w1-photo-interrupter-angle-sensor}

#### フォトインタラプタ {#w1-photo-interrupter}
...
### 三端子レギュレータ {#w1-three-terminal-regulator}
...
実は mbed 基板上にも 3.3V の三端子レギュレータ（LD33 と書かれたチップ）が載っており，右上の VOUT ピンから安定化された 3.3V が出ています（Figure ([Fig.](#fig:mbed_pin)) 参照）．この電圧を使ってもよいのですが，今回のブレッドボード配置ではこのピンにアクセスしづらいため，別途レギュレータを取り付けることにします．

## 並列で進める組立作業 {#w1-parallel-assembly}

以下の 3 つの作業は並列に進めることができます．各作業の成果は，後半のチームによる回路組立と立ち上げで利用します．

### Task A：機械組立（学生A） {#sec:w1-mechanical-assembly}
...
#### 傾斜センサの取り付け {#w1-task-a-angle-sensor-attach}

まず，傾斜センサ（No.6）を車輪付きユニバーサルボード（No.1）の下面に取り付けます．Figure ([Fig.](#fig:angle_sensor)) のように，L 字金具（No.7）を用いて固定してください．
...
#### DC モータの取り付け {#w1-task-a-motor-attach}

次に DC モータ（No.3）を取り付けます．モータブラケット（No.4）を用いて，ユニバーサルボード（No.1）の車軸と同じ面に固定してください（Figure ([Fig.](#fig:motor_setup)) 参照）．ブラケット固定用のねじは少し小さい M2 ねじです．頭が小さく，付属ドライバでは回しにくいことがありますが，緩まないようしっかり締めてください．
...
#### ブレッドボードの取り付け {#w1-task-a-breadboard-attach}
...
#### 電池ボックスの取り付け {#w1-task-a-battery-attach}
...
ここまでで基本的な機械構造は完成です．完成例を Figure ([Fig.](#fig:machine)) に示します．
...
### Task B：マイコンのセットアップ（学生B） {#sec:w1-microcontroller-setup}
...
#### Keil Studio Cloud の準備 {#w1-keil-signup}
...
一般にはこれらの機能を同時に全部使うわけではないので，この基板（および一般的なマイコン）では 1 本のピンに複数機能が割り当てられており，必要に応じてプログラムから切り替えて使います．この対応関係を示したのが Figure ([Fig.](#fig:mbed_pin)) です．

<figure id="fig:mbed_pin">
<div class="center">
<img src="../../figs/pinout.png" alt="mbed LPC1768 pinout." style="max-width: 720px; width: 100%; height: auto;" />
</div>
<figcaption>mbed LPC1768 pinout (from mbed platform page).</figcaption>
</figure>
...
#### 制御プログラムの基本 {#w1-control-program-basics}
...
### Task C：ブレッドボード配線設計（学生C） {#sec:w1-breadboard-wiring-design}

#### ブレッドボードの使い方 {#w1-breadboard-howto}

ブレッドボードは電子回路の試作によく使われる基板で，半田付け不要で簡単に回路を組めます．Figure ([Fig.](#fig:breadboard)) にブレッドボードの概略を示します．ブレッドボードでは横一列に並んだ 5 個のホールが電気的につながっており，これを利用して配線します．左右の 5 個ずつのホールは互いに独立しています．
...
例えば，mbed はすでにブレッドボードに挿してあるはずです．mbed のピンに何かをつなぐには，そのピンのすぐ隣のホールにリード線や抵抗の足を挿すだけです（Figure ([Fig.](#fig:breadboard_sample)) 参照）．
...
Figure ([Fig.](#fig:fritzing_setup_and_examples)) に本演習の回路設計例を示します．パーツ配置と配線を始める前に，Fritzing の *View* メニューから *Align to Grid* のチェックを外してください；グリッドへのスナップは本設定での配置・配線には不便です．また，**モータドライバ部分をあらかじめ配線した** 設計例も用意しています（Figures ([Fig.](#fig:fritzing_motor_driver_schematic)) and ([Fig.](#fig:fritzing_motor_driver_breadboard)))．これを参考にし，詳細な部品一覧と手順は Week 1 の回路組立節を参照してください．Fritzing を使うかどうかに関わらず参照できる完全な回路例も付録[^15]に掲載しています．
...
## 回路組立（チーム作業） {#sec:w1-circuit-assembly}
...
*注：部品番号，仕様，個数の詳細は Appendix [app:partslist](appendix.md#app:partslist) を参照してください．*

#### 配線手順 {#w1-stage-1-wiring}
...
#### サンプルプログラムとデバッグツール {#w1-stage-1-debug-tools}
...
#### 確認チェックリスト {#w1-stage-1-checklist}
...
#### サンプルプログラムとデバッグツール {#w1-stage-2-debug-tools}
...
#### 確認チェックリスト {#w1-stage-2-checklist}
...
### Stage 3：全体回路の統合 {#w1-stage-3}

#### 目標 {#w1-stage-3-objective}

最後の段階では，モータ制御系とセンサ系を統合し，倒立振子を立たせるための完全なフィードバック制御系にします．

#### 最終組立 {#w1-stage-3-final-assembly}
...
- 配線が整頓されている（Figure ([Fig.](#fig:circuit_sample)) 参照）

- ショートの恐れのある配線がない

- すべての部品の向きが正しい（IC，電解コンデンサ）

#### 回路組立の一般的な注意事項 {#w1-stage-3-notes}
...
完成した回路の実装例を Figure ([Fig.](#fig:circuit_sample)) に示します．
...
#### チーム最終課題：倒立実験（エンコーダ追加前） {#sec:w1-standup-pre-encoder}
...
[^3]: Appendix [app:circuit](appendix.md#app:circuit): 回路例．
[^4]: Appendix [app:drv8832](appendix.md#app:drv8832): DRV8832 データシート（Texas Instruments）．
[^5]: Appendix [app:tpr105f](appendix.md#app:tpr105f): TPR-105F データシート．
[^6]: Appendix [app:ld1117v33](appendix.md#app:ld1117v33): LD1117V33 データシート．
[^7]: 機械組立の動画例は course materials repository 内の にあります．
[^8]: 型番，仕様，個数を含む完全な部品表は Appendix [app:partslist](appendix.md#app:partslist) を参照してください．
[^9]: Appendix [app:re280ra](appendix.md#app:re280ra): RE-280RA データシート（Mabuchi Motor）．
...
[^13]: Processing は視覚的プログラミング用の無料オープンソースソフトウェアです．<https://processing.org/> からダウンロードできます．2026 年時点で最新の主要リリースは Processing 4 です．詳細は Appendix [app:processing](appendix.md#app:processing) を参照してください．
[^14]: Fritzing インストーラ（共有 Google Drive）: <https://drive.google.com/drive/folders/1CwJ8srD090W6hOeP39BXLUZOVy883Kn2?usp=sharing>
[^15]: Appendix [app:circuit](appendix.md#app:circuit): 回路例．
[^16]: 回路組立の動画例は course materials repository 内の にあります．
[^17]: Appendix [app:circuit](appendix.md#app:circuit): 回路例．
[^18]: Appendix [app:circuit](appendix.md#app:circuit): 回路例．
[^19]: Appendix [app:circuit](appendix.md#app:circuit): 回路例．
